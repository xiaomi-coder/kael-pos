...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  loginError: "",
  isLoggingIn: false,
  isRestoring: true, // true until we check existing session

  setLoginUser: (val) => set({ loginUser: val }),
  setLoginPass: (val) => set({ loginPass: val }),

  // Called once on app start to restore existing session
  restoreSession: async () => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user) {
        // Fetch the user's profile from our users table using auth_id
  ...
  handleLogin: async () => {
    ...
    try {
      // Map username → fake email format for Supabase Auth
      ...
      // Fetch the user's profile (role, permissions, name) from our users table
