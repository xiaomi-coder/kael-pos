...
export const useStorage = create<StorageState>((set, get) => ({
  ...
  setUsers: async (updater) => {
    ...
    if (err) { set({ users: oldItems }); console.error('syncTable error:', err); }
  },
  ...
  setProducts: async (updater) => {
    ...
    if (err) { set({ products: oldItems }); console.error('syncTable error:', err); }
  },
  setCustomers: async (updater) => {
    ...
    if (err) { set({ customers: oldItems }); console.error('syncTable error:', err); }
  },
  setSales: async (updater) => {
    ...
    if (err) { set({ sales: oldItems }); console.error('syncTable error:', err); }
  },
  setExpenses: async (updater) => {
    ...
    if (err) { set({ expenses: oldItems }); console.error('syncTable error:', err); }
  },
  setDealers: async (updater) => {
    ...
    if (err) { set({ dealers: oldItems }); console.error('syncTable error:', err); }
  },
  setDealerTxns: async (updater) => {
    ...
    if (err) { set({ dealerTxns: oldItems }); console.error('syncTable error:', err); }
  },
